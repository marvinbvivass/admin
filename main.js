...
import { renderVentasSection } from './ventas.js'; // NUEVO: Importa la función de ventas
...
function renderMainAppScreen() {
    ...
    document.getElementById('btn-precios').addEventListener('click', async () => {
        modalContainer.classList.remove('hidden');
        await renderPreciosSection(modalContainer, renderMainAppScreen);
    });

    // MODIFICADO: Event Listener para la sección de Ventas
    document.getElementById('btn-ventas').addEventListener('click', async () => {
        modalContainer.classList.remove('hidden');
        await renderVentasSection(modalContainer, renderMainAppScreen);
    });
