...
const CACHE_NAME = 'gestionapp-cache-v3'; // Nombre de la caché y versión (¡INCREMENTADO LA VERSIÓN!)
const urlsToCache = [
    ...
    '/admin/CargasyVehiculos.js', // ¡Añadido!
    '/admin/main.js', // ¡Añadido!
    ...
    event.respondWith(
        caches.match(event.request).then((response) => {
            ...
            return fetch(event.request).then((networkResponse) => {
                ...
                console.warn('Service Worker: Fallo al cargar recurso desde la red o caché:', event.request.url, error);
                // Devolver una respuesta de error genérica o una página offline de fallback
                return new Response('<h1>Offline</h1><p>No se pudo cargar el recurso y no hay caché disponible.</p>', {
                    status: 503,
                    statusText: 'Service Unavailable',
                    headers: new Headers({ 'Content-Type': 'text/html' })
                });
                // Si tuvieras una página offline dedicada, podrías usar:
